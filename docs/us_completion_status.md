...
| Acceptance Criteria | Status |
|---|---|
| Python function: if `new_model.auc > champion.auc + threshold` → promote to production | Todo |
| New model version visible in MLflow registry with promotion audit trail | Todo |
| Screenshot of registry showing version history across Sprint 2 and Sprint 3 | Todo |
...
| Acceptance Criteria | Status |
|---|---|
| Optuna study with >= 20 trials, each logged as a separate MLflow run | Todo |
| Best hyperparameters logged as MLflow params on winning run | Todo |
| AUC-ROC improvement over Sprint 2 baseline documented | Todo |
| Screenshot of Optuna optimisation curve | Todo |

### US-19 — SHAP Explainability
| Acceptance Criteria | Status |
|---|---|
| SHAP summary plot (feature importance across all validation samples) | Todo |
| SHAP waterfall plot for 3 sample customers (high/medium/low risk) | Todo |
| SHAP values stored per prediction in `processed.churn_predictions` table | Todo |
| Plots logged as MLflow artifacts | Todo |

### US-21 — Automated Retraining DAG
| Acceptance Criteria | Status |
|---|---|
| Airflow DAG: `check_drift_results → retrain_if_needed → evaluate → register` | Todo |
| DAG triggers full retrain only if PSI > 0.2 or AUC delta > 0.05 (from US-14) | Todo |
| Screenshot of DAG with conditional branch logic | Todo |
...
### Dependency Graph
```
US-17 (Optuna) ──→ US-20 (Champion-Challenger)
US-19 (SHAP)   ──→ US-23 (Full Web App)
US-14 (done)   ──→ US-21 (Retraining DAG)
...
- **US-01, 09, 10, 12, 13, 14, 16**: Screenshots needed from running services
